...
- ✅ Error states / loading states / empty states - `AppLoader`/`AppErrorState`/`AppEmptyState` (Phase 6, unit-tested Phase 8) are now wired into the Diet Plan screen, including a genuine error state that was missing entirely (a failed fetch used to render identically to "no diet plan assigned," no retry). Home gets the same loader on its one unified loading spot (the request-status action button); the rest of Home is a composite dashboard of independently-fetched sections (video/quotes/journey each manage their own state) with no single loading/error/empty to wire the shared widgets onto - adopting them per-section there is still open follow-up work. Other screens (meal logging, water, progress, chat, notifications, payment) are untouched.
