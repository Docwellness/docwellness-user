...
class _DietPlanScreenState extends State<DietPlanScreen> with RouteAware {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      // "Your diet plan starts soon" ONLY when today isn't covered by any
      // week the plan has (across every cycle - see backend getActiveDiet,
      // which now prepends completed cycles' weeks too). A renewal whose
      // next cycle starts the day after the last one ends is continuous -
      // some week always covers today - so no countdown; a real gap between
      // cycles (or a brand-new plan starting next week) leaves today
